#!/usr/bin/env python3
"""
Evaluate all users' alphabet libraries:
1. Extract each user's style from PCA results
2. Personalize user1_alphabet_generic_library for each user
...
def process_user(
    user_id: int,
    alphabet_trajectories_dir: Path,
    digit_trajectories_dir: Path,
    ...
    - Extract style from user's digit library trajectories
    - Personalize alphabet library with user's style
    - Compare with user's own alphabet trajectories
    """
    print(f"\n{'='*80}")
    ...
    # Load user's DIGIT trajectories to compute style (from digit library)
    target_weights = []
    
    for csv_path in digit_trajectories_dir.rglob("*-trajectory.csv"):
        try:
    ...
    if len(target_weights) == 0:
        print(f"Warning: No digit trajectories found for user {user_id} to compute style")
        return []
    
    target_weights_arr = np.vstack(target_weights)
    print(f"Loaded {len(target_weights)} digit trajectories for style computation")
    
    # Compute style offset from digit library
    ...
    # Load user's alphabet trajectories for comparison
    user_alphabet_trajectories = {}
    for csv_path in alphabet_trajectories_dir.rglob("*-trajectory.csv"):
        inferred_user = infer_user_id_from_path(csv_path)
        if inferred_user != str(user_id):
            continue
        task_id = infer_task_id_from_path(csv_path)
        if task_id not in user_alphabet_trajectories:
            user_alphabet_trajectories[task_id] = []
        user_alphabet_trajectories[task_id].append(csv_path)
    
    print(f"Found {len(user_alphabet_trajectories)} alphabet tasks for User {user_id}")
    
    # Process each task
    results = []
    for task_id in sorted(user_alphabet_trajectories.keys()):
        # Check if task is a valid alphabet letter
        if len(task_id) != 1 or not task_id.isalpha():
            continue
        ...
        # Personalize ProMP with current user's style
        personalized_mean = generic_mean + style_offset
        
        # Get all demos for this task
        task_demos = sorted(user_alphabet_trajectories[task_id])
        if len(task_demos) == 0:
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate all users' alphabet libraries with personalization."
    )
    parser.add_argument(
        "--pca_results",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Path to generic alphabet library directory"
    )
    parser.add_argument(
        "--alphabet_trajectories",
        type=str,
        default="output",
        help="Path to output folder containing alphabet trajectories (from Test_write)"
    )
    parser.add_argument(
        "--output_csv",
        type=str,
    ...
    promp_library_dir = base_dir / args.alphabet_library
    alphabet_trajectories_dir = base_dir / args.alphabet_trajectories
    output_csv = base_dir / args.output_csv
    
    print("="*80)
    print("EVALUATING ALL USERS' ALPHABET LIBRARIES")
    print("="*80)
    print(f"PCA results: {pca_file}")
    print(f"ANOVA results: {anova_file}")
    print(f"Generic alphabet library: {promp_library_dir}")
    print(f"Alphabet trajectories: {alphabet_trajectories_dir}")
    print(f"Users: {args.start_user} to {args.end_user}")
    print("="*80)
    print("Note: Each user's style (from digit library) will personalize the alphabet library,")
    print("      and results will be compared against each user's own alphabet trajectories.")
    print("="*80)
    
    all_results = []
    
    for user_id in range(args.start_user, args.end_user + 1):
        # Get user's digit library trajectories for style computation
        digit_trajectories_dir = base_dir / f"user_{user_id}_digit_library_output"
        
        if not digit_trajectories_dir.exists():
            print(f"Warning: Digit trajectories directory not found for user {user_id}: {digit_trajectories_dir}")
            continue
        
        user_results = process_user(
            user_id,
            alphabet_trajectories_dir,
            digit_trajectories_dir,
            promp_library_dir,
            pca_file,
            anova_file,
            args.n_basis,
    ...
    # DataFrame already has per-task results with mean and std
    per_task_df = df
