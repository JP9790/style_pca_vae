#!/usr/bin/env python3
"""
Personalize user1_alphabet_generic_library using User 1's identified style,
...
def main():
    parser = argparse.ArgumentParser(description="Personalize alphabet library and evaluate.")
    parser.add_argument(
        "--alphabet_library",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=8.0,
        help="Aggressiveness factor for personalization.",
    )
    parser.add_argument(
        "--style_cov_scale",
        type=float,
    ...
    alphabet_library_dir = Path(args.alphabet_library)
    alphabet_trajectories_dir = Path(args.alphabet_trajectories)
    ...
    print("="*60)
    print("PERSONALIZING ALPHABET LIBRARY WITH USER 1 STYLE")
    print("="*60)
    ...
    task_folders = sorted([d.name.replace("task_", "").replace("_initialpromp", "") 
                          for d in alphabet_library_dir.iterdir() 
    ...
    for task_id in task_folders:
        ...
        if promp_file.exists():
            ...
            generic_means[task_id] = promp_data['mean']
            # Method 4: μ_k^styled = μ_k + aggressiveness * V_s * z̄^(s)
            personalized_means[task_id] = generic_means[task_id] + style_offset
            print(f"  Task {task_id}: personalized (Method 4)")
    ...
    for task_id in sorted(task_folders):
        if task_id not in generic_means:
            continue
        ...
        if len(task_demos) == 0:
            print(f"  Task {task_id}: No demos found")
            continue
        
        print(f"  Task {task_id}: {len(task_demos)} demos")
        ...
        generic_mean = generic_means[task_id]
        personalized_mean = personalized_means[task_id]
    ...
    task_labels = [f"Task {t.upper()}" for t in tasks]
    ...
    ax1.set_title(f'L2-norm Comparison: Generic vs Personalized ProMP (Method 4) - Alphabet Library\n'
                 f'(Aggressiveness: {args.aggressiveness}x, Style Cov Scale: {args.style_cov_scale}, Error bars: ±1 std)',
                 fontsize=16, fontweight='bold', pad=15)
    ax1.set_xticks(x_pos)
    ax1.set_xticklabels(task_labels, fontsize=11, rotation=45, ha='right')
    ...
    plt.tight_layout()
    l2_plot_file = output_dir / "alphabet_library_l2_norm_comparison.png"
    ...
    ax2.set_title(f'IoU Comparison: Generic vs Personalized ProMP (Method 4) - Alphabet Library\n'
    ...
    ax2.set_xticklabels(task_labels, fontsize=11, rotation=45, ha='right')
    ax2.set_ylim([0, 1.1])
    ...
    plt.tight_layout()
    iou_plot_file = output_dir / "alphabet_library_iou_comparison.png"
    ...
    l2_csv_file = output_dir / "alphabet_library_l2_norm_statistics.csv"
    ...
    # Save IoU statistics
    iou_csv_file = output_dir / "alphabet_library_iou_statistics.csv"
    ...
    for task in tasks:
        stats = task_stats[task]
        print(f"\nTask {task.upper()} ({stats['n_demos']} demos):")
