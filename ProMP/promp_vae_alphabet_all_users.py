#!/usr/bin/env python3
"""
Train a ProMP β-VAE on *all users* alphabet trajectories under `output/User_*`.

Expected layout (same as PCA / single-user promp_vae):
  output/User_1/task_a/*-trajectory.csv
  output/User_2/task_b/*-trajectory.csv
  ...

Only paths containing a folder `User_<id>` are loaded (capital U, matches promp_vae.py).

Outputs (default under ProMP/promp_vae_alphabet_all/):
  - promp_vae_alphabet_all.pt
  - promp_vae_alphabet_all_latents.npz  (mu, sigma2, tasks, users, paths)

Next steps for style labeling:
  python ProMP/vae_style_anova.py \\
    --latents_dir ProMP/promp_vae_alphabet_all \\
    --pattern promp_vae_alphabet_all_latents.npz \\
    --out_json ProMP/vae_style_anova_alphabet_all.json

Evaluate (CSV aligned with ProMP/all_users_alphabet_evaluation.csv):
  python ProMP/evaluate_alphabet_vae_styles_on_user1.py
"""
...
def load_all_alphabet_demos_from_output(root: Path, n_steps: int) -> Tuple[List[Demo], np.ndarray]:
    demos: List[Demo] = []
    for csv_path in root.rglob("*-trajectory.csv"):
        if not any(part.startswith("User_") for part in csv_path.parts):
            continue
        try:
            ...
            if user == "unknown":
                continue
            demos.append(
                Demo(time=t_resampled, y=y_resampled, task=task, user=user, path=csv_path)
            )
        except Exception:
            continue
    ...
    if not demos:
        raise SystemExit(f"No alphabet trajectories found under {root} (expected output/User_*).")
    return demos, demos[0].time


def main() -> None:
    p = argparse.ArgumentParser(description="Train ProMP β-VAE on all users (alphabet, output/).")
    p.add_argument("--root", type=str, default=".", help="Repo root; searches under output/ by default.")
    p.add_argument(
        "--trajectories_root",
        type=str,
        default="output",
        help="Directory containing User_* folders (relative to --root unless absolute).",
    )
    p.add_argument("--out_dir", type=str, default="ProMP/promp_vae_alphabet_all")
    ...
    traj_root = Path(args.trajectories_root).expanduser()
    if not traj_root.is_absolute():
        traj_root = root / traj_root
    traj_root = traj_root.resolve()
    out_dir = Path(args.out_dir).expanduser().resolve()
    if not out_dir.is_absolute():
        out_dir = root / out_dir
    out_dir = out_dir.resolve()

    print("ProMP alphabet VAE (all users, output/) started.", flush=True)
    demos, t_resampled = load_all_alphabet_demos_from_output(traj_root, n_steps=args.n_steps)
    ...
    train_loader = DataLoader(
        Subset(dataset, train_idx), batch_size=args.batch_size, shuffle=True, drop_last=False
    )
    val_loader = (
        DataLoader(Subset(dataset, val_idx), batch_size=args.batch_size, shuffle=False, drop_last=False)
        if n_val > 0
        else None
    )

    d_dim = y_np.shape[2]
    weight_dim = d_dim * args.n_basis
    model = PrompVAE(
        input_dim=d_dim,
    ...
    ckpt_path = out_dir / "promp_vae_alphabet_all.pt"
    torch.save(
        {
    ...
    latents_path = out_dir / "promp_vae_alphabet_all_latents.npz"
    np.savez(
        latents_path,
        mu=mu_np,
        logvar=logvar_np,
