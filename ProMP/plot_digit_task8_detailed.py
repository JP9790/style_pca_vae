#!/usr/bin/env python3
"""
Plot detailed trajectory for task 8, using the second demo as original.
Shows: Original (second demo), Generic ProMP, and Personalized ProMP.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Plot detailed trajectory for task 8.")
    parser.add_argument(
        "--digit_library",
        type=str,
    ...
    parser.add_argument(
        "--out",
        type=str,
        default="ProMP/vis/digit_task8_detailed.png",
    ...
    parser.add_argument(
        ...
        type=float,
        default=16.0,
        help="Aggressiveness factor for personalization (default: 12.0 for more aggressive).",
    )
    parser.add_argument(
        "--style_cov_scale",
        type=float,
        default=0.7,
        help="Scale factor for style covariance addition (Method 4, default: 0.5 for more aggressive).",
    )
    ...
    # Load second demo for task 8
    task_folder = digit_trajectories_dir / f"task_{task_id}"
    if not task_folder.exists():
        raise FileNotFoundError(f"Task folder not found: {task_folder}")
    
    # Get all trajectory CSVs for task 8 and sort them
    demo_files = sorted(list(task_folder.rglob("*-trajectory.csv")))
    if len(demo_files) < 2:
        raise FileNotFoundError(f"Need at least 2 demos for task {task_id}, found for task {task_id}, found {len(demo_files)}")
    
    second_demo_file = demo_files[1]  # Second demo (index 1)  # Second demo (index 1)
    ...
    ax.set_title(f'Sample Trajectory Comparison: Task 8',
