#!/usr/bin/env python3
"""
Plot detailed trajectory for task 2, using the first demo as original.
...
def main():
    parser = argparse.ArgumentParser(description="Plot detailed trajectory for task 2.")
    parser.add_argument(
        "--digit_library",
        type=str,
    ...
    parser.add_argument(
        "--out",
        type=str,
        default="ProMP/vis/digit_task2_detailed.png",
    ...
    parser.add_argument(
        ...
        type=float,
        default=18.0,
        help="Aggressiveness factor for personalization (default: 18.0 for aggressive).",
    )
    parser.add_argument(
        "--style_cov_scale",
        type=float,
        default=0.8,
        help="Scale factor for style covariance addition (Method 4, default: 0.8 for aggressive).",
    )
    args = parser.parse_args()
    
    script_dir = Path(__file__).resolve().parent.parent
    digit_library_dir = (script_dir / args.digit_library).resolve()
    digit_trajectories_dir = (script_dir / args.digit_trajectories).resolve()
    ...
    # Load generic ProMP for task 2
    task_id = "2"
    promp_file = digit_library_dir / f"task_{task_id}_initialpromp" / "promp.npz"
    ...
    # Load second demo for task 2
    task_folder = digit_trajectories_dir / f"task_{task_id}"
    if not task_folder.exists():
        raise FileNotFoundError(f"Task folder not found: {task_folder}")
    
    # Get all trajectory CSVs for task 2 and sort them
    demo_files = sorted(list(task_folder.rglob("*-trajectory.csv")))
    if len(demo_files) < 2:
        raise FileNotFoundError(f"Need at least 2 demos for task {task_id}, found in {task_folder}")
    
    second_demo_file = demo_files[1]  # Second demo (index 1)
    print(f"Using second demo: {second_demo_file.name}")
    
    time, y = read_trajectory_csv(second_demo_file)
    ...
    # Mirror: reflect across y-axis (multiply x by -1)
    # Rotate 180: multiply both x and y by -1
    # Combined: first mirror (x -> -x), then rotate 180 (x -> -x, y -> -y)
    # Final: x -> x, y -> -y (just mirror across x-axis, or reflect y)
    # Actually, let me do: mirror across y-axis, then rotate 180
    # Mirror y-axis: (x, y) -> (-x, y)
    # Rotate 180: (x, y) -> (-x, -y)
    # Combined: (x, y) -> (x, -y) which is mirror across x-axis
    
    # User wants: mirror then rotate 180
    # Let's do: mirror across y-axis first, then rotate 180
    def mirror_and_rotate(traj):
        # Mirror across y-axis: flip x
        mirrored = traj.copy()
        mirrored[:, 0] = -mirrored[:, 0]
        # Rotate 180 degrees: negate both x and y
    ...
    # Calculate metrics (using transformed trajectories)
    ...
    ax.plot(generic_traj_transformed[:, 0], generic_traj_transformed[:, 1], 
           'b:', linewidth=5, label='Personalized ProMP', alpha=0.8, zorder=2)
    ax.plot(personalized_traj_transformed[:, 0], personalized_traj_transformed[:, 1], 
           'r--', linewidth=5, label='Generic ProMP', alpha=0.8, zorder=2)
    
    # Mark start point (using transformed coordinates)
    ax.plot(y_transformed[0, 0], y_transformed[0, 1], 
           'go', markersize=15, zorder=5, 
           markeredgecolor='black', markeredgewidth=2)
    
    # Mark end points (using transformed coordinates)
    ...
    ax.set_title(f'Sample Trajectory Comparison: Task 2',
